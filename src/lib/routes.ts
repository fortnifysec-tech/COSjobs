 ...
 */
export type RouteGroup = "open" | "transfer" | "scheme";

export const ROUTE_GROUPS: { key: RouteGroup; title: string; lede: string }[] = [
  { key: "open", title: "Routes you can apply for from the open jobs market", lede: "A licensed employer offers you a job and assigns a certificate. These are the routes a job seeker can actually use." },
  { key: "transfer", title: "Transfers within a business", lede: "For people who already work for the overseas arm of the sponsor. Not open to applicants from outside the company." },
  { key: "scheme", title: "Schemes and agreements", lede: "Sponsored through an approved scheme operator or under an international agreement, for a fixed purpose and period." },
];

export type RouteInfo = {
  name: string;
  group: RouteGroup;
  /** Can a person be hired into this route from the open market? */
  openMarket: boolean;
  /** Does time on the route count towards settlement? */
  settlement: "yes" | "no" | "some";
  /** Longest single grant, in plain words. */
  maxStay: string;
...
export const ROUTE_INFO: RouteInfo[] = [
  ...
  },
  {
    ...
    openMarket: false,
    settlement: "no",
    maxStay: "5 years in any 6 (9 for high earners)",
  ...
  },
  {
    ...
    openMarket: false,
    settlement: "no",
    maxStay: "12 months, extendable to 2 years",
  ...
  },
  {
    ...
    openMarket: false,
    settlement: "no",
    maxStay: "12 or 24 months, by scheme",
